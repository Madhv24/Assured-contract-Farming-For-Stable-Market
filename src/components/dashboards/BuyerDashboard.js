...
const BuyerDashboard = () => {
  ...
  const fetchContracts = async () => {
    try {
      if (profile?._id) {
        const response = await axios.get('/api/contracts/buyer/' + profile._id);
  ...
  const openCreateContract = (farmer) => {
  ...
  const sendInterestToFarmer = async (farmerId) => {
    try {
      await axios.post(`/api/buyers/interest-in-farmer/${farmerId}`);
      ...
      // Refresh profile to update interested farmers list
      await fetchProfile();
    } catch (error) {
  ...
  const tabs = [
    ...
    { id: 'progress', label: 'Progress Tracking', icon: <TrendingUp className="w-5 h-5" /> },
    { id: 'farmers', label: 'All Farmers', icon: <Users className="w-5 h-5" /> },
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto py-6 px-4 sm:px-6 lg:px-8">
        ...
        {activeTab === 'contracts' && (
          <div className="space-y-6">
            <div className="card">
              <div className="card-header">
                <h3 className="text-lg font-medium text-gray-900">My Contracts</h3>
              </div>
              <div className="card-body">
                {contracts.length > 0 ? (
                  <div className="space-y-4">
                    {selectedContract ? (
                      <div>
                        ...
                        <div className="text-sm text-gray-600 mb-4">
                          <p>Farmer: {selectedContract.farmerId?.name || selectedContract.farmerId?.companyName}</p>
                        ...
                        {selectedContract.status === 'Pending' && (
                          <div className="p-4 bg-yellow-50 text-yellow-700 rounded">
                            ...
                            {!selectedContract.buyerApproved && (
                              <div className="mt-3">
                                <button
                        ...
                        {selectedContract.status === 'Active' && (
                          <div className="p-4 bg-green-50 text-green-700 rounded">
                            Contract is active! You can now track farmer progress.
                          </div>
                        )}
                      </div>
                    ) : (
        ...
        {activeTab === 'progress' && (
          <div className="space-y-6">
            <div className="card">
              <div className="card-header">
                <h3 className="text-lg font-medium text-gray-900">Farmer Progress Tracking</h3>
              </div>
              <div className="card-body">
                {contracts.length > 0 ? (
                  <div className="space-y-4">
                    {selectedContract ? (
                      <div>
                        <div className="flex justify-between items-center mb-4">
                          <h4 className="text-lg font-medium text-gray-900">
                            Progress for: {selectedContract.cropName} ({selectedContract.quantity} {selectedContract.unit})
                          </h4>
                          <button
                            onClick={() => setSelectedContract(null)}
                            className="btn-outline text-sm"
                        ...
                        <div className="text-sm text-gray-600 mb-4">
                          <p>Farmer: {selectedContract.farmerId?.name || selectedContract.farmerId?.companyName}</p>
                          <p>Farm Location: {selectedContract.farmerId?.farmDetails?.location || 'Not specified'}</p>
                          <p>Expected Delivery: {new Date(selectedContract.expectedDeliveryDate).toLocaleDateString()}</p>
                        </div>
                        {selectedContract.status === 'Active' ? (
                          <ProgressUpdates
                            ...
                            onUpdate={() => {
                              ...
                              axios.get(`/api/progress/contracts/${selectedContract._id}`)
                                .then(response => setSelectedContract(response.data.contract))
                                .catch(error => console.error('Error fetching contract:', error));
                            }}
                          />
                        ) : (
                          <div className="p-4 bg-yellow-50 text-yellow-700 rounded">
                            Contract is pending approval by both parties. Tracking will unlock once Active.
                            <div className="mt-3">
                              <button
                                onClick={async () => {
                                  try {
                                    await axios.post(`/api/contracts/${selectedContract._id}/approve`);
                      ...
                      <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                        {contracts.map((contract, index) => (
                          <div key={index} className="border rounded-lg p-4 hover:shadow-md transition-shadow cursor-pointer"
                            ...
                            <div className="flex justify-between items-start mb-3">
                              <div>
                                <h4 className="font-medium text-gray-900">{contract.cropName}</h4>
                                <span className="text-sm text-gray-500">{contract.quantity} {contract.unit}</span>
                              </div>
                              <span className={`px-2 py-1 rounded-full text-xs font-medium ${
                                contract.status === 'Active' ? 'bg-green-100 text-green-800' : 
                            ...
                            <div className="space-y-2 text-sm text-gray-600 mb-4">
                              <p>Farmer: {contract.farmerId?.name || contract.farmerId?.companyName}</p>
                              <p>Farm Location: {contract.farmerId?.farmDetails?.location || 'Not specified'}</p>
                              <p>Price: ₹{contract.price} {contract.priceUnit}</p>
                              <p>Delivery: {new Date(contract.expectedDeliveryDate).toLocaleDateString()}</p>
                              <p>Progress: {contract.progressUpdates?.filter(p => p.status === 'Completed').length || 0} of {contract.progressUpdates?.length || 0} stages</p>
                            </div>
                    ...
                    {filteredFarmers.map((farmer, index) => (
                      <div key={index} className="border rounded-lg p-4 hover:shadow-md transition-shadow">
                        ...
                        <div className="flex items-center justify-between text-xs text-gray-500 mb-2">
                          <span className={`status-badge ${farmer.isAvailable ? 'status-active' : 'status-cancelled'}`}>
                            {farmer.isAvailable ? 'Available' : 'Not Available'}
                          </span>
                          <span>{(farmer.images?.length || 0)} images • {(farmer.contractPapers?.length || 0)} docs</span>
                        </div>
                        <div className="flex space-x-2">
                          <button
                            onClick={() => openProfileDetail(farmer, 'farmer')}
                    ...
                    {profile.interestedFarmers.map((interest, index) => (
                      <div key={index} className="border rounded-lg p-4">
                        ...
                        {interest.status === 'accepted' && (
                          <div className="mt-3">
                            <button
                              onClick={() => openCreateContract(interest.farmerId)}
                              className="btn-primary text-sm"
                            >
                              Create Contract
                            </button>
      ...
      {isCreateContractOpen && selectedFarmerForContract && (
        <CreateContractModal
          isOpen={isCreateContractOpen}
          onClose={closeCreateContract}
          onSubmit={handleCreateContract}
          farmer={selectedFarmerForContract}
          userRole="buyer"
        />
      )}
    </div>
